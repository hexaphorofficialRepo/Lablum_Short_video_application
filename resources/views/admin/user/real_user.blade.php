            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-header-title">
                        <h4 class="pull-left page-title" style="font-size: 30px;"> Real/Active Users </h4>
                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td> <a href="{{route('admin.user.wallet.record',['user_id' => $user->user_id])}}" class="btn btn-secondary history-btn"
                                        ...
                                        {{-- <td> <a href="{{route('admin.reported.account',['user_id' => $user->user_id])}}" class="btn btn-secondary history-btn"
                                                style="background-image: linear-gradient(352deg,#af0888,#131597)!important;color:white;"
                                              >
                                                <i class="fas fa-piggy-bank" style="margin-right: 5px;"></i>Report Account
                                        </a>
                                        </td> --}}
                                    </tr>
                                @endforeach
                ...
                <form action="{{route('admin.block.user')}}" method="POST" enctype="multipart/form-data">
                   @csrf
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="blockReason" name="block_reason" rows="4" placeholder="Enter reason for blocking"></textarea>
                    </div>
                    <input type="hidden" id="blockedUserId" name="user_id"> <!-- Hidden input for user_id -->
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        $('.feature-toggle').on('change', function() {
            ...
            $('#blockedUserId').val(userId); // Update the hidden input with the user_id
            $('#blockUserModal').modal('show');
        });

        $('#blockUserForm').submit(function(event) {
